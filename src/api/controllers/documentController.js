...
export default {
    ...
    updateDocument: async (req, res, next) => {
        try {
            ...
            const pdfFileName = `${owner?.phoneNumber.replace("+", "")}-${
                plot?._id
            }`;
            ...
            if (owner?.oldNumbers?.length > 0) {
                plotDetailForm = {
                    ...plotDetailForm,
                    oldNumbers:
                        owner?.oldNumbers[owner?.oldNumbers?.length - 1],
                };
            }
            ...
            await createOwnerCertificate({
                ...
                return next({
                    error_code: 400,
                    error_message:
                        error?.message || error || "Error while creating PDF",
                });
            });
            logger.apiInfo(
                req,
                res,
                `Created PDF file ${pdfFileName} successfully!`
            );
            await encryptPdf({
                fileName: pdfFileName,
                did: documentDid,
    ...
    createPlotCertificationMongo: async (req, res, next) => {
        try {
    ...
    getLastestVersion: async (req, res, next) => {
        try {
            ...
            return res
                .status(200)
                .json(
                    deepUnsalt(endorsementChain[endorsementChain.length - 1])
                );
        } catch (error) {
            return next({
                error_code: 400,
